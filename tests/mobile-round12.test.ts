...
describe("Workspace tab strip mobile usability", () => {
  test("per-tab close buttons are touch-visible (hover-reveal only on sm+)", async () => {
    ...
    expectNoTokens(tabs, ["opacity-0 transition-opacity hover:bg-accent hover:text-foreground group-hover:opacity-100"]);
    // Touch-visible by default, hover-reveal restored on sm+.
    expectTokens(tabs, ["sm:opacity-0", "sm:group-hover:opacity-100"]);
    expectTokens(tabs, ["opacity-100", "transition-opacity", "hover:bg-accent", "hover:text-foreground"]);
  ...
  test("a close-others affordance is offered once the strip is crowded", async () => {
    ...
    expectTokens(tabs, ["tabs.length > 3"]);
    expectTokens(tabs, ["Close all tabs except"]);
  });
  ...
  test("the store exposes closeOtherTabs and keeps the requested tab active", async () => {
    ...
    expectTokens(types, ["closeOtherTabs: (id: string) => void;"]);
    const ui = await source("src/lib/rdash/store/slices/ui.ts");
    expectTokens(ui, ["closeOtherTabs: (id) =>"]);
    expectTokens(ui, ["tabs: [keep]"]);
  });
});
...
describe("Attendance policy rendering", () => {
  test("verification rules and policy form use the canonical normalized policy", async () => {
    ...
    expectTokens(mod, ['import { normalizeAttendancePolicy } from "@/lib/rdash/attendance-policy"']);
    ...
    expectNoTokens(mod, ["grace_minutes: 15"]);
    expectNoTokens(mod, ["half_day_hours: 4"]);
...
describe("Record select-mode pills", () => {
  test("Select / Exit select pills never wrap mid-label on phones", async () => {
    for (const path of [
      ...
      expectTokens(src, ["inline-flex", "shrink-0"]);
    }
  });
});
