...
describe("workspace bootstrap and scoped client reads", () => {
  ...
  test("loads the reusable Master foundation once in bootstrap", async () => {
    ...
    expectTokens(projectedBootstrap, ["fullCollections: [...WORKSPACE_FOUNDATION_COLLECTIONS]"]);
    expectNoTokens(projectedBootstrap, ["bounded compatibility read"]);
  ...
  test("hydrates the foundation bootstrap without loading operational workspace tables", async () => {
    ...
    expectTokens(bootstrap, ["data: workspace.data"]);
    expectTokens(bootstrap, ['readStrategy: "foundation-first"']);
  });
  ...
  test("prevents modules from rendering unloaded arrays as authoritative zero values", async () => {
    ...
    expectTokens(boundary, ["workspaceReadState.recordResponse(response, requestedTarget)"]);
  });
  ...
  test("does not present omitted scoped collections as real zero totals", async () => {
    ...
    expectTokens(app, ['loadedCollections && !loadedCollections.has(collection) ? "—" : String(count)']);
    expectTokens(app, ['collectionCount("customers", db.customers.length)']);
    expectTokens(app, ['collectionCount("workOrders", db.workOrders.length)']);
    expectTokens(app, ['collectionCount("purchaseOrders", db.purchaseOrders.length)']);
  });
  ...
  test("does not present unloaded notification sources as authoritative zero alerts", async () => {
    ...
    expectTokens(notifications, ['strategy !== "row"']);
    expectTokens(notifications, ["module details load on demand"]);
    expectNoTokens(notifications, ["notificationCoverageComplete && unread.length > 0"]);
    ...
    expectTokens(notifications, ["unread.length > 0 ?"]);
    expectTokens(notifications, ["Notification data will fill in as relevant modules load."]);
    expectTokens(notifications, ["All caught up! No pending alerts."]);
    expectTokens(notifications, ["filterCoverageComplete ?"]);
  });
  ...
  test("routes the mobile plus launcher through authoritative scopes and canonical forms", async () => {
    ...
    expectTokens(sheet, ['pendingLoadState?.status === "loaded"']);
  ...
  test("preserves module permissions and response telemetry on dedicated endpoints", async () => {
    ...
    expectTokens(helper, ["target?.scope === endpointTarget.scope"]);
    expectTokens(helper, ["getModuleScopedWorkspace(user, target)"]);
  ...
  test("normal workspace reads have no full-workspace fallback or fallback feature flag", async () => {
    ...
    expectTokens(route, ['"X-UC-Read-Architecture": "scoped-only"']);
    expectTokens(route, ["trying the module graph"]);
  });
