...
-- Final hard cutover: the application must now use canonical Storage Account
-- IDs and uc_google_drive_credentials exclusively. Re-copy the latest encrypted
-- credentials once more in case the old production build reauthorized a Drive
-- between the preparation migration and this final cleanup.
do $guard$
declare
  v_payload jsonb;
begin
...
-- No CASCADE: an unexpected dependency must abort rather than silently remove
-- something the new architecture still needs.
