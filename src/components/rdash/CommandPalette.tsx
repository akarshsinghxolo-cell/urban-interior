...
export function CommandPalette() {
    ...
    // CRON-7: Search history
    const [history, setHistory] = React.useState<string[]>([]);
    React.useEffect(() => {
        try {
            const stored = localStorage.getItem("uc_cmd_history");
    ...
    const addToHistory = (term: string) => {
        ...
        setHistory(next);
        try { localStorage.setItem("uc_cmd_history", JSON.stringify(next)); } catch { /* non-fatal */ }
    };
          ...
          {q.trim() === "" && history.length > 0 ? (
              <div className="px-2.5 py-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {history.map((term, i) => (
                    <button key={i} type="button" onClick={() => { setQ(term); setActiveIdx(0); }} className="rounded-md border border-border bg-muted/40 px-2 py-1 text-[11px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
                      {term}
                    </button>
                  ))}
                </div>
