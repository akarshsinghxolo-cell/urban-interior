...
export function ArticleVendorAssetLinks({ articleId, vendorId, variantId, title = "Linked catalogues" }: {
    ...
    return <div className="flex flex-wrap items-center gap-1.5"><span className="text-[10px] text-muted-foreground">{title}:</span>{matches.slice(0, 2).map(({ catalogue, file }) => file ? <button key={catalogue.id} type="button" className="w-12 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" title="Open media context" onClick={() => openDetail("media" as any, file.id)}><FilePreview file={assetPreview(file)} compact controls className="h-12"/></button> : <a key={catalogue.id} href={catalogue.catalog_url || "#"} target="_blank" rel="noreferrer" className="inline-flex max-w-[150px] items-center gap-1 truncate rounded border border-primary/20 bg-primary/[0.04] px-1.5 py-0.5 text-[10px] text-primary hover:bg-primary/10"><BookOpen className="h-3 w-3 shrink-0"/><span className="truncate">{catalogue.title}</span></a>)}{matches.length > 2 && <Button size="sm" variant="ghost" className="h-5 px-1 text-[10px]" onClick={() => setActiveModule("mediaCommunication")}>+{matches.length - 2}</Button>}</div>;
}
