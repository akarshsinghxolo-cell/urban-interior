...
export type FilePreviewSource = {
    ...
    url?: string;
    thumbnailUrl?: string;
    /** Authenticated server proxy base (see /api/google-drive/stream). Serves private Drive files. */
    proxyUrl?: string;
};
...
export function managedPreviewUrl(file: FilePreviewSource) {
    if (file.proxyUrl) return `${file.proxyUrl}&mode=media`;
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] max-w-6xl overflow-hidden p-0 sm:max-w-6xl">
        ...
        <div className="max-h-[calc(100vh-9rem)] min-h-72 overflow-auto bg-muted/30 p-3">
          ...
          {publicDrivePreview && file.proxyUrl && kind === "image" && previewUrl ? <img src={previewUrl} alt={file.fileName} className="mx-auto max-h-[calc(100vh-12rem)] max-w-full rounded-md object-contain"/> : null}
          {publicDrivePreview && file.proxyUrl && kind === "video" && previewUrl ? <video src={previewUrl} controls preload="metadata" className="mx-auto max-h-[calc(100vh-12rem)] max-w-full rounded-md bg-black"/> : null}
          {publicDrivePreview && file.proxyUrl && kind === "pdf" && previewUrl ? <iframe title={`Preview ${file.fileName}`} src={previewUrl} className="h-[calc(100vh-12rem)] min-h-[32rem] w-full rounded-md border border-border bg-white"/> : null}
          {publicDrivePreview && file.proxyUrl && kind === "document" ? <div className="flex min-h-72 flex-col items-center justify-center gap-3 text-center"><IconForKind kind={kind}/><p className="max-w-sm text-sm text-muted-foreground">This file does not have an inline preview. Download it or open it in Google Drive.</p></div> : null}
...
export function FilePreview({ file, className, compact = false, controls: _controls = true, onOpen, interactive = true }: FilePreviewProps) {
    ...
    const thumbnailUrl = !proxyFailed ? managedThumbnailUrl(file, compact ? 320 : 640) : (() => {
        // Proxy failed — retry with the public Drive thumbnail or local thumbnail before showing the fallback card.
        if (file.googleFileId) return publicDriveThumbnailUrl(file.googleFileId, compact ? 320 : 640);
        return file.thumbnailUrl || (fileKind(file) === "image" ? file.url || "" : "");
    })();
    ...
    React.useEffect(() => { setThumbnailFailed(false); setProxyFailed(false); }, [sourceKey]);
    const open = () => {
        if (onOpen)
            return onOpen();
        if (hasInlinePreview)
