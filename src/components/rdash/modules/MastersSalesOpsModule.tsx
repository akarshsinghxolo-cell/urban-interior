...
export function MastersModule({ submodule }: {
    ...
    if (effectiveSubmodule === "staff") {
        ...
        const registerDocument = () => {
            ...
            toast.success("Staff document uploaded as pending verification");
        };
        return (<div className="flex flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="rounded-[var(--panel-radius)] border border-border bg-card p-4 shadow-card">
          <div className="mb-3 flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-bold">Staff document upload & verification</p><p className="text-[11px] text-muted-foreground">Register Google Drive staff files, link them to file assets, then verify, reject, expire or remove them.</p></div><StatusBadge label={`${staffDocuments.length} documents`} className="bg-success/10 text-success border-success/20"/></div>
          <div className="grid gap-2 md:grid-cols-6">
            ...
            <label className="inline-flex h-9 cursor-pointer items-center justify-center gap-1 rounded-md border border-input bg-muted/30 px-3 text-xs font-semibold hover:bg-muted"><FileUp className="h-3.5 w-3.5"/> Choose file<input type="file" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) setDocDraft((value) => ({ ...value, fileName: file.name, mimeType: file.type || undefined, fileSizeBytes: file.size })); }}/></label>
            <input value={docDraft.fileName} onChange={(event) => setDocDraft((value) => ({ ...value, fileName: event.target.value }))} placeholder="File name" className="h-9 rounded-md border border-input bg-card px-3 text-xs md:col-span-4"/>
            <Button size="sm" className="md:col-span-2" onClick={registerDocument}><FileText className="mr-1 h-3.5 w-3.5"/> Register upload</Button>
          </div>
