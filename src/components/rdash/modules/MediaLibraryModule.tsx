    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-[var(--panel-radius)] border border-border bg-card shadow-card">
        ...
        <div className="divide-y divide-border">
          {activeLinks.map((item) => { const asset = catalogues.find((catalogue) => catalogue.id === item.catalogue_id); const article = articles.find((entry) => entry.id === item.article_id); const vendor = vendors.find((entry) => entry.id === item.vendor_id); const driveFile = asset?.drive_asset_id ? files.find((entry) => entry.id === asset.drive_asset_id) : undefined; return <div key={item.id} className="grid gap-2 px-4 py-3 md:grid-cols-[72px_minmax(160px,1.2fr)_minmax(160px,1fr)_minmax(150px,1fr)_auto]"><div>{driveFile ? <FilePreview file={{ fileName: driveFile.file_name, mimeType: driveFile.mime_type, googleFileId: driveFile.storage_mode === "managed" ? driveFile.google_file_id : undefined, url: driveFile.web_view_link, thumbnailUrl: driveFile.thumbnail_url }} compact controls className="h-16" /> : <div className="flex h-16 items-center justify-center rounded-md border border-border bg-muted/20 text-primary"><BookOpen className="h-5 w-5" /></div>}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{asset?.title || "Missing catalogue"}</p><p className="text-[10px] text-muted-foreground">Click the thumbnail to preview</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Article</p><p className="truncate text-xs font-medium">{article?.name || "Missing article"}</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Vendor scope</p><p className="truncate text-xs font-medium">{vendor?.name || "Shared / any vendor"}</p>{item.notes ? <p className="mt-0.5 text-[10px] text-muted-foreground">{item.notes}</p> : null}</div><div className="flex items-center gap-1">{driveFile ? <Button size="sm" variant="outline" className="h-7 px-2 text-[10px]" onClick={() => onOpenFile(driveFile.id)}>Context</Button> : null}<Button size="icon" variant="ghost" aria-label="Archive catalogue link" onClick={() => archiveLink(item.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div></div>; })}
