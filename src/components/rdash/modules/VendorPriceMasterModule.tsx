...
export function VendorPriceMasterModule({ vendorId }: { vendorId?: string } = {}) {
  ...
  const saveRate = async (change: () => void, message: string) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Rate changes could not be saved.");
      return false;
  ...
  const rows = React.useMemo(() => {
    ...
    return master.vendorRates.filter((rate) => {
      if (vendorId && rate.vendor_id !== vendorId) return false;
      ...
      return !needle || `${vendor?.name || ""} ${vendor?.city || ""} ${article?.name || (master.articles.find((row) => row.id === rate.article_id)?.name || "Unknown Article")} ${variant?.name || ""} ${rate.status || "active"}`.toLowerCase().includes(needle);
    });
  }, [master, query, vendorId]);
  ...
  const addPrice = async () => {
    ...
    if (["inactive", "blacklisted"].includes(vendor.status || "")) return toast.error("Activate this vendor before adding rates.");
    ...
    const saved = await saveRate(() => mutateMaster((current) => createInitialVendorRate(current, {
      vendorId: vendor.id,
    ...
    })), `Quoted rate saved for ${selectedArticle.name}.`);
    if (saved) setDraft((current) => emptyDraft(current.vendorId));
  };
  ...
  const editAmount = (rate: VendorRate, rawValue: string) => {
    ...
    useRDashStore.getState().logAudit({
      ...
      action: `Vendor quoted rate edited: ${(master.articles.find((row) => row.id === rate.article_id)?.name || "Unknown Article")}`,
      entity_type: "vendorRate",
      entity_id: rate.id,
      entity_label: (master.articles.find((row) => row.id === rate.article_id)?.name || "Unknown Article"),
      kind: "update",
  ...
  const updateStatus = (rate: VendorRate, status: "active" | "inactive") => {
    const timestamp = now();
    void saveRate(() => mutateMaster((current) => ({
      ...current,
  ...
  const deleteRate = async (rateId: string) => {
    ...
    if (!ok) return;
    await saveRate(() => mutateMaster((current) => ({ ...current, vendorRates: current.vendorRates.filter((rate) => rate.id !== rateId) })), "Vendor rate removed.");
  };
  ...
  const exportRows = () => {
    ...
    rows.forEach((rate) => {
      ...
      const article = master.articles.find((row) => row.id === rate.article_id);
      const variant = master.articleVariants.find((row) => row.id === rate.variant_id);
      data.push([vendor?.name || "", vendor?.city || "", article?.name || (master.articles.find((row) => row.id === rate.article_id)?.name || "Unknown Article"), variant?.name || "Base Article", unitLabel(master, rate.article_id, rate.variant_id), vendorQuotedRate(rate), rate.status || "active", rate.updated_at || ""]);
    });
  ...
  return <div className="flex flex-col gap-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-center gap-2.5"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 text-primary"><Truck className="h-5 w-5" /></span><div><h2 className="text-lg font-bold tracking-tight">Vendor Price Matrix</h2><p className="text-xs text-muted-foreground">Manage agreed article and variant prices. Units and tax settings come from the article master.</p></div></div><Button size="sm" variant="outline" onClick={exportRows}><Download className="mr-1 h-3.5 w-3.5" />Export</Button></div>
    ...
    <section className="rounded-[var(--panel-radius)] border border-border bg-card p-4 shadow-card"><div><h3 className="text-sm font-bold">Add quoted rate</h3><p className="mt-0.5 text-xs text-muted-foreground">No validity dates and no duplicated Unit/GST/commercial calculation fields are stored here.</p></div><div className={vendorId ? "mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2" : "mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-5"}><Field label="Vendor"><select className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" aria-label="Vendor" disabled={Boolean(vendorId)} value={draft.vendorId} onChange={(event) => setDraft((current) => ({ ...current, vendorId: event.target.value }))}><option value="">Select Vendor</option>{master.vendors.filter((vendor) => (!vendorId || vendor.id === vendorId) && !["inactive", "blacklisted"].includes(vendor.status || "")).map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name} · {vendor.city || "No city"}</option>)}</select></Field><Field label="Article"><select className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" value={draft.articleId} onChange={(event) => setDraft((current) => ({ ...current, articleId: event.target.value, variantId: "" }))}><option value="">Select Article</option>{master.articles.map((article) => <option key={article.id} value={article.id}>{article.name}</option>)}</select></Field><Field label="Variant"><select className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" disabled={!draft.articleId} value={draft.variantId} onChange={(event) => setDraft((current) => ({ ...current, variantId: event.target.value }))}><option value="">Base Article</option>{variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.name}</option>)}</select></Field><Field label="Derived unit"><div className="flex h-9 items-center rounded-md border border-input bg-muted/40 px-3 text-sm font-semibold">{draft.articleId ? unitLabel(master, draft.articleId, draft.variantId || undefined) : "Select Article"}</div></Field><Field label="Quoted rate"><div className="flex gap-2"><Input type="number" min="0" step="0.01" value={draft.rate} onChange={(event) => setDraft((current) => ({ ...current, rate: event.target.value }))} /><Button type="button" disabled={saving} onClick={() => void addPrice()}>Add</Button></div></Field></div>{selectedVendor && selectedArticle && <div className="mt-3 rounded-lg border border-border bg-muted/15 px-3 py-2 text-[11px] text-muted-foreground">Vendor capability: {capability ? <><strong className="text-foreground">matched</strong>{capability.brand ? ` · ${capability.brand}` : ""}{capability.availability ? ` · ${capability.availability.replaceAll("_", " ")}` : ""}{capability.typical_lead_time_days != null ? ` · ${capability.typical_lead_time_days} day lead` : ""}{capability.moq != null ? ` · MOQ ${capability.moq}` : ""}</> : <><strong className="text-warning">not recorded</strong> · rate can still be captured, but add the Article to Vendor supply capability for better recommendation quality.</>}</div>}</section>

    <section className="rounded-[var(--panel-radius)] border border-border bg-card p-4 shadow-card"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-bold">Current quoted rates</h3><p className="text-xs text-muted-foreground">Historical source and changes remain in Vendor Rate History; this table contains only current values.</p></div><div className="relative w-full sm:w-72"><Search className="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Vendor / Article / Variant" className="pl-8" /></div></div>{rows.length ? vendorId ? <div className="mt-3 space-y-2">{rows.map((rate) => { const article = master.articles.find((row) => row.id === rate.article_id); const variant = master.articleVariants.find((row) => row.id === rate.variant_id); const isCheapest = cheapest.has(rate.id); return <div key={rate.id} className="rounded-lg border border-border bg-background p-3"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="break-words text-sm font-semibold">{article?.name || (master.articles.find((row) => row.id === rate.article_id)?.name || "Unknown Article")}</p><p className="mt-0.5 break-words text-[11px] text-muted-foreground">{variant?.name || "Base Article"} · {unitLabel(master, rate.article_id, rate.variant_id)}</p></div>{isCheapest && <StatusBadge label="Lowest" className="shrink-0 border-success/20 bg-success/10 text-success" />}</div><div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_9rem_auto] sm:items-end"><Field label="Quoted rate"><Input key={`${rate.id}-${vendorQuotedRate(rate)}`} type="number" min="0" step="0.01" disabled={saving} aria-label="Edit quoted rate" defaultValue={vendorQuotedRate(rate)} className="h-9 w-full font-mono font-bold" onBlur={(event) => editAmount(rate, event.target.value)} /></Field><Field label="Status"><select className="h-9 w-full rounded-md border border-input bg-background px-2 text-xs" disabled={saving} aria-label="Rate status" value={rate.status === "inactive" ? "inactive" : "active"} onChange={(event) => updateStatus(rate, event.target.value as "active" | "inactive")}><option value="active">Active</option><option value="inactive">Inactive</option></select></Field><Button size="icon" variant="outline" className="h-9 w-9 text-destructive" disabled={saving} aria-label="Delete rate" onClick={() => void deleteRate(rate.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div><p className="mt-2 text-[10px] text-muted-foreground">Updated {rate.updated_at ? new Date(rate.updated_at).toLocaleString("en-IN") : "—"}</p></div>; })}</div> : <div className="mt-3 overflow-x-auto rd-scroll"><table className="w-full min-w-[920px] text-left text-xs"><thead><tr className="border-b border-border text-[10px] uppercase tracking-wide text-muted-foreground"><th className="px-2 py-2">Vendor</th><th className="px-2 py-2">Article</th><th className="px-2 py-2">Variant</th><th className="px-2 py-2">Derived unit</th><th className="px-2 py-2">Quoted rate</th><th className="px-2 py-2">Status</th><th className="px-2 py-2">Updated</th><th className="px-2 py-2"></th></tr></thead><tbody>{rows.map((rate) => { const vendor = master.vendors.find((row) => row.id === rate.vendor_id); const article = master.articles.find((row) => row.id === rate.article_id); const variant = master.articleVariants.find((row) => row.id === rate.variant_id); const isCheapest = cheapest.has(rate.id); return <tr key={rate.id} className="border-b border-border/60 last:border-0"><td className="px-2 py-2"><div className="font-semibold">{vendor?.name || rate.vendor_id}</div><div className="text-[10px] text-muted-foreground">{vendor?.city || ""}</div></td><td className="px-2 py-2 font-semibold">{article?.name || (master.articles.find((row) => row.id === rate.article_id)?.name || "Unknown Article")}</td><td className="px-2 py-2">{variant?.name || "Base Article"}</td><td className="px-2 py-2 text-muted-foreground">{unitLabel(master, rate.article_id, rate.variant_id)}</td><td className="px-2 py-2"><div className="flex items-center gap-2"><Input key={`${rate.id}-${vendorQuotedRate(rate)}`} type="number" min="0" step="0.01" disabled={saving} aria-label="Edit quoted rate" defaultValue={vendorQuotedRate(rate)} className="h-8 w-28 font-mono font-bold" onBlur={(event) => editAmount(rate, event.target.value)} />{isCheapest && <StatusBadge label="Lowest" className="border-success/20 bg-success/10 text-success" />}</div></td><td className="px-2 py-2"><select className="h-8 rounded-md border border-input bg-background px-2 text-xs" disabled={saving} aria-label="Rate status" value={rate.status === "inactive" ? "inactive" : "active"} onChange={(event) => updateStatus(rate, event.target.value as "active" | "inactive")}><option value="active">Active</option><option value="inactive">Inactive</option></select></td><td className="px-2 py-2 text-[10px] text-muted-foreground">{rate.updated_at ? new Date(rate.updated_at).toLocaleString("en-IN") : "—"}</td><td className="px-2 py-2 text-right"><Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" disabled={saving} aria-label="Delete rate" onClick={() => void deleteRate(rate.id)}><Trash2 className="h-3.5 w-3.5" /></Button></td></tr>; })}</tbody></table></div> : <div className="mt-3"><EmptyState title="No Vendor rates" description="Add the first current quoted rate above." /></div>}</section>
  </div>;
}
