...
interface CustomerWorkRequiredDialogProps {
  open: boolean;
  customerId: string;
  site: Site;
...
/**
 * Canonical Customer-owned Area + Work Required creation workflow.
 * Area creation, category/subcategory/work-type selection, validation and
 * mutation live here. Site Execution passes Site/Area context only.
 */
export function CustomerWorkRequiredDialog({ open, customerId, site, initialAreaIds, onOpenChange, onCreated }: CustomerWorkRequiredDialogProps) {
  ...
  const save = () => {
    ...
    if (!draft.areaIds.length) return toast.error("Select or create at least one covered Area.");
    try {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>{site.name} · canonical Customer Area / Work Required workflow.</DialogDescription>
        </DialogHeader>
        <WorkRequiredFields
          db={db}
          site={site}
