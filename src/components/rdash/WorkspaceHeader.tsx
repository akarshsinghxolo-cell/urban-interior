...
export function WorkspaceHeader() {
  ...
  const toggleSidebar = useRDashStore((state) => state.toggleSidebar);
  const sidebarCollapsed = useRDashStore((state) => state.sidebarCollapsed);
  ...
  const setMoreMenuOpen = useRDashStore((state) => state.setMoreMenuOpen);
  ...
  return (
    <header className="sticky top-0 z-30 flex flex-col gap-2 border-b border-border bg-background/85 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3 px-[var(--page-pad)] py-2.5">
        ...
        <Button variant="outline" size="icon" className="relative hidden h-11 w-11 shrink-0 md:inline-flex" onClick={() => setKeyboardShortcutsOpen(true)} aria-label="Show keyboard shortcuts" title="Keyboard shortcuts (?)">
        ...
        {hasPendingChanges ? (
          ...
          <span className="hidden text-[10px] font-medium text-muted-foreground lg:inline">Saving…</span>
        ) : workspaceSyncStatus === "error" ? (
          <span title={workspaceSyncError || "Server synchronization failed"} className="hidden text-[10px] font-semibold text-destructive lg:inline">Sync failed</span>
        ) : null}
