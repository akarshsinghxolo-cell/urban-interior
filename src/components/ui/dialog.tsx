...
function DialogContent({ className, children, showCloseButton = true, ...props }: React.ComponentProps<typeof DialogPrimitive.Content> & {
    ...
    return (<DialogPortal data-slot="dialog-portal">
      <DialogOverlay />
      <DialogPrimitive.Content data-slot="dialog-content" className={cn("fixed z-50 grid w-full gap-4 border bg-background shadow-lg duration-200", "max-h-[92dvh] overflow-y-auto rd-scroll", "inset-x-0 bottom-0 top-auto rounded-t-2xl rounded-b-none p-4 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:slide-in-from-bottom data-[state=closed]:slide-out-to-bottom data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0", "sm:inset-auto sm:top-[50%] sm:left-[50%] sm:max-w-[calc(100%-2rem)] sm:max-w-lg sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-lg sm:rounded-b-lg sm:p-6 sm:data-[state=open]:zoom-in-95 sm:data-[state=closed]:zoom-out-95", className)} {...props}>
        <div aria-hidden="true" className="mx-auto -mt-1 mb-1 h-1.5 w-10 shrink-0 rounded-full bg-muted-foreground/30 sm:hidden"/>
        {children}
