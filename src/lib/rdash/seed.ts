...
const measurementRevisions: MeasurementRevision[] = [
    ...
    { id: "measure-das-master-paint-v1", site_id: "site-das-apartment", area_id: "area-das-master", work_required_id: "work-das-paint", revision_no: 1, length: 12, width: 11, height: 9, unit: "ft", calculated_area: 132, calculated_perimeter: 46, notes: "Verified paintable wall basis for the Master Bedroom.", captured_by: "Ravi Kumar", captured_at: at(-14), photo_count: 3, status: "verified" },
...
const workRequired: WorkRequired[] = [
    { id: "work-das-ceiling", customer_id: "cust-das", site_id: "site-das-apartment", area_ids: ["area-das-master"], title: "Gypsum False Ceiling", work_category_id: "fc", work_subcategory_ids: ["fc_gyp"], description: "12.5 mm gypsum board, GI framework, suspension system and jointing finish. Master Bedroom ceiling execution package.", status: "in_progress", source: "Referral", priority: "high", budget: 54000, created_at: at(-21), updated_at: now() },
    { id: "work-das-paint", customer_id: "cust-das", site_id: "site-das-apartment", area_ids: ["area-das-master", "area-das-guest"], title: "Interior Painting", work_category_id: "pw", work_subcategory_ids: ["pw_int"], description: "Putty correction, primer and two finish coats. Can be quoted room-wise or as an apartment package without creating duplicate work required records.", status: "contractor_bidding", source: "Referral", priority: "high", budget: 42000, created_at: at(-20), updated_at: now() },
    { id: "work-das-wardrobe", customer_id: "cust-das", site_id: "site-das-apartment", area_ids: ["area-das-master"], title: "Wardrobe / TV Unit", work_category_id: "fc2", work_subcategory_ids: ["fc2_ward", "fc2_tv"], description: "BWP carcass, laminate shutters, soft-close hardware. Awaiting final design selection.", status: "quotation_in_progress", source: "Referral", priority: "medium", budget: 125000, created_at: at(-19), updated_at: now() },
    { id: "work-das-office-ceiling", customer_id: "cust-das", site_id: "site-das-office", area_ids: ["area-das-reception"], title: "Grid False Ceiling", work_category_id: "fc", work_subcategory_ids: ["fc_grid"], description: "600×600 acoustic tiles with GI T-grid. Office-only scope.", status: "measurement_done", source: "Direct", priority: "high", budget: 86000, created_at: at(-9), updated_at: now() },
    { id: "work-das-office-paint", customer_id: "cust-das", site_id: "site-das-office", area_ids: ["area-das-reception"], title: "Office Interior Painting", work_category_id: "pw", work_subcategory_ids: ["pw_int"], description: "Surface preparation, primer and finish coats. Office-only paint scope.", status: "new", source: "Direct", priority: "medium", budget: 32000, created_at: at(-8), updated_at: now() },
    { id: "work-aarav-kitchen", customer_id: "cust-aarav", site_id: "site-aarav-home", area_ids: ["area-aarav-kitchen"], title: "Modular Kitchen", work_category_id: "fc2", work_subcategory_ids: ["fc2_kit"], description: "BWP carcass, acrylic shutters, soft-close hardware. Customer quotation is sent.", status: "quotation_sent", source: "Instagram", priority: "high", budget: 180000, created_at: at(-12), updated_at: now() },
    { id: "work-nisha-wardrobe", customer_id: "cust-nisha", site_id: "site-nisha-home", area_ids: ["area-nisha-bedroom"], title: "Wardrobe / TV Unit", work_category_id: "fc2", work_subcategory_ids: ["fc2_ward", "fc2_tv"], description: "To be defined after site measurement.", status: "new", source: "Walk-in", priority: "medium", budget: 145000, created_at: at(-6), updated_at: now() },
];
...
const invoices: CustomerInvoice[] = [
    { id: "inv-das-ceiling-advance", invoice_no: "INV-2026-101", finance_context: "service", customer_id: "cust-das", site_id: "site-das-apartment", quotation_id: "quote-das-ceiling", work_order_id: "wo-das-ceiling", payment_id: "pay-das-ceiling-advance", title: "Gypsum ceiling advance", status: advanceReceived === advanceAmount ? "paid" : "partial", subtotal: advanceAmount, tax_amount: 0, total_amount: advanceAmount, paid_amount: advanceReceived, balance_amount: advanceAmount - advanceReceived, issued_at: date(-12), due_date: date(-12), created_at: at(-12), updated_at: now() },
    { id: "inv-aarav-kitchen-advance", invoice_no: "INV-2026-103", finance_context: "service", customer_id: "cust-aarav", site_id: "site-aarav-home", work_required_id: "work-aarav-kitchen", quotation_id: "quote-aarav-kitchen", title: "Modular kitchen booking advance", status: "paid", subtotal: 25000, tax_amount: 0, total_amount: 25000, paid_amount: 25000, balance_amount: 0, issued_at: date(0), due_date: date(0), paid_at: date(0), created_at: at(0), updated_at: now() },
];
...
const workOrderCostLines: WorkOrderCostLine[] = [
    ...
    { id: "cost-das-contractor-accrual", work_order_id: "wo-das-ceiling", type: "contractor", description: "Sharma Ceiling Works — verified RA bill (50% progress)", amount: 14500, date: at(-1), source_kind: "bill", source_id: "cbill-das-ceiling", vendor_id: "con-gypsum", vendor_name: "Sharma Ceiling Works", created_at: at(-1) },
...
const seedDrawings: Drawing[] = [
    { id: "drawing-das-ceiling-v1", drawing_no: "DRW-2026-001", title: "Master Bedroom Ceiling Layout", kind: "2D", site_id: "site-das-apartment", site_name: "Das Residence — 3BHK Apartment", area_id: "area-das-master", area_name: "Master Bedroom", work_order_id: "wo-das-ceiling", work_order_no: "WO-2026-301", version: 1, status: "approved", uploaded_by: "Anita Rao", uploaded_at: at(-9), approved_by: "Akarsh Singh", approved_at: at(-8), notes: "Initial 2D ceiling layout showing grid + cove placement.", created_at: at(-9), updated_at: now() },
...
const entityFileAttachments = [
    ...
    { id: "file-link-quote-reference", file_asset_id: "file-reference-ceiling-joint", entity_type: "quotation" as const, entity_id: "quote-das-ceiling", entity_label: "Q-2026-201", role: "other" as const, visibility: "customer" as const, customer_shareable: true, created_by: "Owner", created_at: at(-9), updated_at: now() },
];
const seedAuditLog = [
