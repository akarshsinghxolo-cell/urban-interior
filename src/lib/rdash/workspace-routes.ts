...
export interface WorkspaceRouteDefinition {
  ...
  legacyModuleIds?: readonly string[];
}
...
const ROUTE_DEFINITIONS: readonly WorkspaceRouteDefinition[] = [
  ...
  { moduleId: "boqControlCentre", canonicalPath: "/workspace/procurement/boq", legacyModuleIds: ["boq"] },
  ...
  { moduleId: "dispatch", canonicalPath: "/workspace/procurement/dispatch" },
  {
    ...
    canonicalPath: "/workspace/contractors",
    aliases: ["/workspace/contractorDetail"],
    legacyModuleIds: ["contractorPerformance", "contractors"],
  },
  { moduleId: "contractorRates", canonicalPath: "/workspace/contractors/rates" },
  {
    moduleId: "vendors",
    canonicalPath: "/workspace/vendors",
    legacyModuleIds: ["vendorPerformance"],
  },
  ...
  { moduleId: "contractorPayments", canonicalPath: "/workspace/finance/contractor-bills" },
  {
    ...
    canonicalPath: "/workspace/finance/profitability",
    legacyModuleIds: ["siteProfitability", "workOrderPnl"],
  },
  ...
  {
    moduleId: "hrStaff",
    canonicalPath: "/workspace/staff",
    aliases: ["/workspace/hrStaff"],
    legacyModuleIds: ["staff"],
  },
  ...
  {
    moduleId: "salesAnalytics",
    canonicalPath: "/workspace/reports/sales",
    legacyModuleIds: ["salesReport", "quotationConversion", "leadSourceReport"],
  },
  {
    moduleId: "collectionAnalytics",
    canonicalPath: "/workspace/reports/collections",
    legacyModuleIds: ["collectionReport", "agingReportRep"],
  },
  {
    moduleId: "operationsAnalytics",
    canonicalPath: "/workspace/reports/operations",
    legacyModuleIds: ["staffProductivity", "visitCompliance", "taskThroughput"],
  },
  {
    moduleId: "financialAnalytics",
    canonicalPath: "/workspace/reports/financial",
    legacyModuleIds: ["jobPnlReport", "vendorExposureReport", "taxReport"],
  },
...
function buildWorkspaceRouteRegistry() {
  ...
  const legacyModuleIds = new Map<string, string>();

  const registerPath = (path: string, definition: WorkspaceRouteDefinition, isAlias: boolean) => {
  ...
  for (const rawDefinition of ROUTE_DEFINITIONS) {
    const definition = Object.freeze({
      ...
      aliases: Object.freeze([...(rawDefinition.aliases || [])].map(normalizeWorkspacePath)),
      legacyModuleIds: Object.freeze([...(rawDefinition.legacyModuleIds || [])]),
    });
    ...
    registerPath(legacyIdPath(definition.moduleId), definition, definition.canonicalPath !== legacyIdPath(definition.moduleId));
    for (const alias of definition.aliases || []) registerPath(alias, definition, true);
    for (const legacyModuleId of definition.legacyModuleIds || []) {
      const existing = legacyModuleIds.get(legacyModuleId);
      if (existing && existing !== definition.moduleId) {
        throw new Error(`Legacy module id ${legacyModuleId} maps to both ${existing} and ${definition.moduleId}`);
      }
      legacyModuleIds.set(legacyModuleId, definition.moduleId);
  ...
  return {
    ...
    byPath: byPath as ReadonlyMap<string, { definition: WorkspaceRouteDefinition; isAlias: boolean }>,
    legacyModuleIds: legacyModuleIds as ReadonlyMap<string, string>,
  };
}

const REGISTRY = buildWorkspaceRouteRegistry();

export const WORKSPACE_ROUTE_DEFINITIONS = Object.freeze([...REGISTRY.byModuleId.values()]);

export function workspacePathForModule(moduleId: string): string {
  const canonicalModuleId = REGISTRY.legacyModuleIds.get(moduleId) || moduleId;
  return REGISTRY.byModuleId.get(canonicalModuleId)?.canonicalPath
    || REGISTRY.byModuleId.get(DEFAULT_MODULE_ID)?.canonicalPath
    || WORKSPACE_ROOT_PATH;
}

export function resolveWorkspacePath(pathname: string): WorkspaceRouteMatch | undefined {
  ...
  return {
    ...
    matchedPath,
    isAlias: match.isAlias || matchedPath !== match.definition.canonicalPath,
  };
}
...
export function isWorkspacePath(pathname: string): boolean {
  ...
  return normalized === WORKSPACE_ROOT_PATH || normalized.startsWith(`${WORKSPACE_ROOT_PATH}/`);
}

export function validateWorkspaceRouteRegistry(): string[] {
  const issues: string[] = [];
  const visibleModuleIds = [...MODULE_ROUTE_REGISTRY.values()]
    .filter((route) => !route.hidden)
    .map((route) => route.id);

  for (const moduleId of visibleModuleIds) {
    if (!REGISTRY.byModuleId.has(moduleId)) {
      issues.push(`Visible module route ${moduleId} has no canonical workspace URL.`);
    }
  }

  for (const definition of REGISTRY.byModuleId.values()) {
    const route = MODULE_ROUTE_REGISTRY.get(definition.moduleId);
    if (!route) issues.push(`Workspace URL ${definition.canonicalPath} points to unknown module ${definition.moduleId}.`);
    if (!definition.canonicalPath.startsWith(WORKSPACE_ROOT_PATH)) {
      issues.push(`Workspace URL for ${definition.moduleId} must start with ${WORKSPACE_ROOT_PATH}.`);
    }
    if (definition.canonicalPath !== normalizeWorkspacePath(definition.canonicalPath)) {
      issues.push(`Workspace URL for ${definition.moduleId} is not normalized.`);
    }
  }

  for (const [legacyModuleId, canonicalModuleId] of REGISTRY.legacyModuleIds) {
    if (!MODULE_ROUTE_REGISTRY.has(legacyModuleId)) {
      issues.push(`Legacy module id ${legacyModuleId} is not registered by the internal module registry.`);
    }
    if (!REGISTRY.byModuleId.has(canonicalModuleId)) {
      issues.push(`Legacy module id ${legacyModuleId} points to missing canonical module ${canonicalModuleId}.`);
    }
  }
