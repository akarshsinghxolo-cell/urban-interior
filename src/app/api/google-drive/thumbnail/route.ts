...
export async function GET(request: NextRequest) {
    try {
        ...
        // Legacy local-file references still resolve for existing records, but the active
        // upload pipeline requires Google Drive and does not create new local uploads.
        ...
        const metadataResponse = await fetch(
            `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?fields=name,mimeType,thumbnailLink`,
        ...
        if (metadata.thumbnailLink) {
            ...
            // Keep authorization on Vercel, but let Google Drive deliver the thumbnail
            // bytes directly to the browser so Vercel is not a media proxy.
            const redirect = NextResponse.redirect(thumbnail.toString(), 307);
            redirect.headers.set("Cache-Control", "private, max-age=300");
            return redirect;
        }

        return new NextResponse(placeholder(metadata.name || "Managed file", metadata.mimeType || ""), {
