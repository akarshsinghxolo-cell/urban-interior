...
export async function GET(request: NextRequest) {
  ...
  try {
    const workspace = await getModuleScopedWorkspace(user, workspaceReadTargetForModule("siteExecution"));
    return NextResponse.json({
      ...
      headers: {
        ...
        "Server-Timing": `workspace-sites;dur=${workspace.loadMs.toFixed(2)}`,
      },
    });
  } catch (error) {
    ...
    console.error("[api/sites] module-scoped read failed:", error);
    return NextResponse.json(
      { error: "Site workspace data is temporarily unavailable." },
